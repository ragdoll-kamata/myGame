...
std::unique_ptr<CardCommand::ParseBoolResult> CardCommand::ParseBool(std::vector<std::string>& boolTokens) {
	...
	// スペースはコマンド化の時はスキップされてるからパースを通してスペース分けであっても問題なくする
...
int CardCommand::CalculationIntExprNode(std::unique_ptr<ExprNode>& root, Card* card) {
	...
	if (root->type == IntExprNodeType::Multiply) {
		return left * right;
	}
...
void CardCommand::ExprNodeSet(std::unique_ptr<ExprNode>& root, std::unique_ptr<ExprNode>& node) {
	...
	if (root->left == nullptr || root->left->type == IntExprNodeType::None) {
	...
	if (root->right == nullptr || root->right->type == IntExprNodeType::None) {
