...
void Object3dCommon::IfSkinningDraw(bool is) {
	if (is != isSkinning) {
		ID3D12GraphicsCommandList* commandList = directXCommon_->GetCommandList();
		if (is) {
			commandList->SetPipelineState(skinningGraphicsPipelineState.Get());
		} else {
			commandList->SetPipelineState(graphicsPipelineState.Get());
		}
		isSkinning = is;
...
void Object3dCommon::CreateGraphicsPipeline()
{
	...
	Microsoft::WRL::ComPtr<IDxcBlob> vertexShaderBlob =
		directXCommon_->CompileShader(L"resource/shaders/Object3d.VS.hlsl", L"vs_6_0");
	assert(vertexShaderBlob != nullptr);

	Microsoft::WRL::ComPtr<IDxcBlob> pixelShaderBlob =
		directXCommon_->CompileShader(L"resource/shaders/Object3d.PS.hlsl", L"ps_6_0");
...
void Object3dCommon::CreateSkinningGraphicsPipeline() {
	...
	Microsoft::WRL::ComPtr<IDxcBlob> vertexShaderBlob =
		directXCommon_->CompileShader(L"resource/shaders/SkinningObject3d.VS.hlsl", L"vs_6_0");
	assert(vertexShaderBlob != nullptr);
	...
	//
	HRESULT hr = directXCommon_->GetDevice()->CreateGraphicsPipelineState(&graphicsPipelineStateDesc,
		IID_PPV_ARGS(&skinningGraphicsPipelineState));
	assert(SUCCEEDED(hr));
}
