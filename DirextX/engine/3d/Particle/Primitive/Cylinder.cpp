...
void Cylinder::Initialize (uint32_t division, float radius, float height) {
	...
	for (uint32_t i = 0; i <= division; i++) {
		...
		vertexData[i * 2 + 0].position = {radius * sin, 0.0f, radius * cos, 1.0f};
		vertexData[i * 2 + 0].texcoord = {u, 0.0f};
		vertexData[i * 2 + 1].position = {radius * sin, height, radius * cos, 1.0f};
...
void Cylinder::CreateVertex (int division) {
...
void Cylinder::CreateIndex (int division) {
